...
(define (same-parity . items)
  (define parity-f (if (even? (car items))
    even?
    (lambda (x) (not (even? x)))))

  (define (iter items parity-items)
    (cond ((null? items) parity-items)
          ((parity-f (car items)) (cons (car items) (iter (cdr items) parity-items)))
          (else (iter (cdr items) parity-items))))
